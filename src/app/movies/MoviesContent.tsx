...
import { getMovieGenres, getFilteredMovies, Movie, Genre } from '@/lib/tmdb';
...
export default function MoviesPage() {
  ...
  const fetchMovies = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getFilteredMovies(currentPage, selectedGenreIds.join(','), selectedYear, selectedSort);
      
      // Filter out movies with explicit keywords after fetching
      const safeMovies = data.results.filter(movie =>
        !containsExplicitKeyword(movie.title) &&
        !containsExplicitKeyword(movie.overview)
      );
      ...
      <main className="flex-grow pt-20">
        <div className="container mx-auto px-4 md:px-8 lg:px-12">
          ...
          {!loading && !error && movies.length === 0 && (
            <div className="flex justify-center items-center h-48">
              <p className="text-textMuted">No movies found for this selection.</p>
            </div>
          )}
