...
import { getTVShowDetails, getPrimaryVideoKey, getImageUrl, getTVShowRecommendations, CrewMember, CastMember, getYouTubeWatchUrl } from '@/lib/tmdb';
...
export default async function TVShowDetailsPage({ params }: { params: { id: string } }) {
  ...
  // FIX: Handle the case where tvShow.videos is undefined by passing null
  ...
  const cast = tvShow.credits?.cast?.filter((member: CastMember) => member.profile_path).slice(0, 10); // Show top 10 cast members with a photo
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start space-y-8 md:space-y-0 md:space-x-10">
            ...
            <div className="flex-shrink-0 w-56 h-auto transition-transform duration-300 transform hover:scale-105">
              {tvShow.poster_path ? (
                <Image
                  ...
                  width={224}
                  height={336}
                  // REMOVED: border-2 border-secondaryBg
            ...
            <div className="text-center md:text-left mt-4 md:mt-0">
             ...
             <div className="flex justify-center md:justify-start gap-4 mt-6">
                <Link
                    href={`/watch/tv/${id}`}
                    // The updated className starts here
